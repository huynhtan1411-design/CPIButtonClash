...
namespace UISystems
{
    ...
    public static class Utils
    {
        ...
        public static void EnableCG(CanvasGroup cg, bool islock = true)
        {
            //cg.alpha = 1;
            cg.DOFade(1, 0.6f).SetUpdate(true);
            cg.interactable = true;
            cg.blocksRaycasts = islock;
            foreach (var ui in cg.GetComponentsInChildren<PopOut>())
            {
                ui.PlayShowAnim();
            }
            foreach (var ui in cg.GetComponentsInChildren<DOTweenAnimation>())
            {
                ui.DOPlay();
            }
        }
        ...
        public static void HideAllCGs(CanvasGroup[] cgs, CanvasGroup[] toShows)
        {
            for (int i = 0; i < cgs.Length; i++)
            {
                for (int j = 0; j < toShows.Length; j++)
                {
                    if (cgs[i] == toShows[j])
                        EnableCG(cgs[i]);
                    else
