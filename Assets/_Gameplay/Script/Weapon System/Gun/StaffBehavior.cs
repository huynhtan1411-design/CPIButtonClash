...
{
    public class StaffBehavior : BaseGunBehavior
    {
                ...
                && hitInfo.collider.gameObject.layer == PhysicsHelper.LAYER_ENEMY)
            {
                if (Vector3.Angle(shootDirectionFlat, forwardFlat) < 40f)
                {
                    ...
                    for (int i = 0; i < bulletsNumber; i++)
                    {
                        ...
                        bullet.Initialise(damageValue, bulletSpeedValue, elementType, characterBehaviour.ClosestEnemyBehaviour, -1f, false,
                           explosionRadius, 1f);
                    }
